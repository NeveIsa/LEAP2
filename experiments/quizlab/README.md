...
- **`grade(quiz_file, question_id, answer)`** — Grades a single question and returns the result.

Browse all functions at `/static/functions.html?exp=quizlab`.

## Quiz Format

Place `.md` files in `ui/quiz/`. Each file uses YAML frontmatter followed by questions separated by `---`.
...
Every question starts with a level-2 heading that includes a number and a unique ID:
...
4. See immediate feedback: correct/incorrect and an explanation.
5. Previously submitted answers are saved and shown on reload.
